...
"""NVIDIA Parakeet / Nemotron streaming speech-to-text service.

Vendored from the YC Voice Agents Hackathon starter (pipecat-ai/yc-voice-agents-hackathon,
server/nvidia_stt.py, BSD-2-Clause, (c) Daily). It speaks the hackathon Nemotron ASR fleet's
websocket protocol (ws://…:8080 — {"type":"ready"} / {"type":"transcript"} / {"type":"reset"}),
which no built-in Pipecat service covers.

OTTO COMPAT NOTE: the upstream file targets pipecat 1.x; this agent is pinned to 0.0.108
(bot.py uses the 0.0.x APIs). All base methods it needs exist in 0.0.108 — the ONLY gap is
three newer-pipecat instance attrs (`_reconnecting`, `_muted`, `_reconnect_audio_buffer`) read
by process_audio_frame. We initialize them in __init__ (see "compat shim"), which makes the
reconnect/mute path a no-op (never reconnecting, never muted) — exactly the 0.0.108 behavior.
"""
...
class NVidiaWebSocketSTTService(WebsocketSTTService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── OTTO compat shim (pipecat 0.0.108) ───────────────────────────────
        # process_audio_frame (below) reads these three attrs; pipecat 1.x sets
        # them in the base class, 0.0.108 doesn't. Initialize them here so the
        # reconnect/mute branch is a harmless no-op on our pinned version.
